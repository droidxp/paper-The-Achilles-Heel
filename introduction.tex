...
version of an app is a malware. These limitations compromise a full understanding of the \mas performance. We present more details about the \mas and related work in Section~\ref{sec:background}.

\fh{To better understand the impact of these issues on previously published results, this paper proposes an evaluation of the accuracy of \mas by replicating the study conducted by Bao et al.~\cite{DBLP:conf/wcre/BaoLL18}. The goal of this replication is to verify the original study's findings and reassess the \mas using DroidBot [29]. DroidBot is a test case generation tool that, according to Bao et al., provides the most accurate results for Android sandboxes in terms of malware detection. At a second moment, we repeat the replication, but this time using a curated dataset of app pairs (original/repackaged versions), which is significantly larger than the dataset used in Bao et al.'s study. Our new dataset contains \apps pairs of original and repackaged apps. The datasets and the procedures for data collection and analysis are detailed in Section~\ref{sec:experimentalSetup}.}
